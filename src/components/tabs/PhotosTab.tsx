...
// ─── Uploaded photo card ───────────────────────────────────────────────────────
function PhotoCard({ photo, areaNames = [], onDelete, onUpdate }: {
  photo: Photo
  areaNames?: string[]
        ...
        {editingDetails ? (
          <div style={{ marginBottom: 8 }}>
            ...
            <div style={{ display: 'flex', gap: 4, marginBottom: 10, flexWrap: 'wrap' }}>
              {CATEGORIES.map(c => (
                <button key={c.value} type="button" onClick={() => setCategoryDraft(c.value)} style={{
            ...
            <div style={{ display: 'flex', gap: 4 }}>
              <button className="btn btn-primary" style={{ fontSize: 11, padding: '5px 10px' }} type="button" onClick={saveDetails}>Save</button>
              <button className="btn btn-ghost" style={{ fontSize: 11, padding: '5px 10px' }} type="button" onClick={cancelEdit}>Cancel</button>
            </div>
          </div>
        ) : (
          <>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 6 }}>
              ...
              <button onClick={openEdit} title="Edit category & note"
