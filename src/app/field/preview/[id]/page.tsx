 ...
 *
 * Usage: open in a separate browser window alongside the Access settings tab.
 * After saving cap changes, hit Refresh in this window to see them applied.
 *
 * URL: /field/preview/[person_id]
 *
 * Guards:
 *   - Requires authenticated admin session
 *   - 404 if person not found in same org
 *
 * Renders the same FieldJobCard list and DetailsTab (readOnly) components that
 * real field workers use, so layout/behaviour is identical.
 */
'use client'
...
export default function FieldPreviewPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      // Person profile + their app access caps
      ...
      setJobs(allJobs.filter(j => ACTIVE_STATUSES.includes(j.status)))
    ...
    return (
      <div style={{ minHeight: '100dvh', background: 'var(--bg)', paddingBottom: 40 }}>
        ...
        <div style={{ background: 'var(--bg)', borderBottom: '1px solid var(--border)', padding: '12px 16px 0', position: 'sticky', top: 48, zIndex: 9 }}>
          ...
          <div style={{ display: 'flex', overflowX: 'auto', borderBottom: '1px solid var(--border)', marginLeft: -16, marginRight: -16, paddingLeft: 16, gap: 0 }}>
            {visibleTabs.map(t => (
  ...
  return (
    <div style={{ minHeight: '100dvh', background: 'var(--bg)', color: 'var(--text)', paddingBottom: 48 }}>
      ...
      {/* Field header — mirrors the real field page */}
      <div style={{ padding: '24px 20px 20px', borderBottom: '1px solid var(--border)' }}>
        ...
        <div style={{ marginTop: 10, display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          <span style={{ fontSize: 11, padding: '3px 10px', borderRadius: 99, fontWeight: 700, background: role === 'admin' ? 'rgba(255,107,53,0.12)' : role === 'manager' ? 'rgba(139,92,246,0.12)' : 'rgba(96,165,250,0.12)', color: role === 'admin' ? '#FF6B35' : role === 'manager' ? '#8B5CF6' : '#60A5FA' }}>
      ...
      <div style={{ padding: '20px 16px' }}>
        ...
        {otherJobs.length > 0 && (
          <section>
            {todayJobs.length > 0 && (
              <div style={{ fontSize: 11, fontWeight: 800, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 12 }}>Active Jobs</div>
            )}
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
  ...
  return (
    <div style={{
      ...
      <button
        ...
        style={{ fontSize: 12, fontWeight: 700, padding: '4px 12px', borderRadius: 6, background: 'rgba(255,255,255,0.1)', border: 'none', color: '#fff', cursor: 'pointer' }}
      >
        ↻ Refresh
      </button>
    </div>
