/*
 * app/api/people/[id]/invoices/[invoiceId]/send/route.ts
 *
 * POST /api/people/[id]/invoices/[invoiceId]/send
 *
 * Sends the invoice as a professional HTML email to ACCOUNTS_EMAIL
 * (falls back to NOTIFY_EMAIL). Marks the invoice status as 'sent'.
 *
 * Admin only.
 */
...
export async function POST(
  ...
  // Fetch invoice + person + job + company
  const [{ data: invoice }, { data: person }, { data: company }] = await Promise.all([
    supabase.from('subcontractor_invoices')
      ...
      .eq('id', invoiceId).eq('person_id', id).single(),
    supabase.from('people').select('name, email').eq('id', id).single(),
    supabase.from('company_profile').select('name').eq('org_id', orgUser.org_id).single(),
  ])
